...
# pipeline_vcf_paired_hg19.sh
# author : brett maroni-rana
# title : Tumor-Normal Mode, Single Patient (sample) Somatic Variant and Germline Caller - GRCh37
# 20211129

# run with sbatch pipeline_vcf_paired_hg19.sh
...
#SBATCH -J vcf_paired19
#SBATCH --partition=campus-new
#SBATCH -c 8
#SBATCH -t 7-12
#SBATCH -o pipeline_vcf_paired_hg19_log_%j.out
# #SBATCH -e pipeline_vcf_paired_hg19_error_%j.out

fastqfolder="/fh/fast/grady_w/users/bmaroni-rana/variant_calling/gatk/tempus/fastq/paired"
outfolder="/fh/fast/grady_w/users/bmaroni-rana/variant_calling/gatk/tempus/data_out_hg19/paired"
...
# run somatic calling in tumor-normal mode
bam_files=(`find ${outfolder} -maxdepth 2 -name "*.sorted.markDup.recal.bam" -type f | sort`)
for ((i=0; i<${#bam_files[@]}; i+=2)); do
	sample=$(echo ${bam_files[i]} | rev | cut -d "/" -f1 | rev | cut -d "." -f1 | cut -d "_" -f1 )
	norm=${sample}_N
	tumor=${sample}_T
	echo "=== call somatic variants for sample ${tumor} ==="
	module load GATK/4.1.8.1-GCCcore-8.3.0-Java-11
	# mutect2 tumor-normal normal
	gatk --java-options "-Xmx20G -XX:+UseParallelGC -XX:ParallelGCThreads=${SLURM_JOB_CPUS_PER_NODE}" \
		Mutect2 \
    	-R ${refgenome} \
    	-I ${outfolder}/${tumor}/${tumor}.sorted.markDup.recal.bam \
    	-I ${outfolder}/${norm}/${norm}.sorted.markDup.recal.bam \
    	-tumor ${tumor} \
    	-normal ${norm} \
		-O ${outfolder}/${tumor}/${tumor}.soma.paried.vcf.gz \
		--f1r2-tar-gz ${outfolder}/${tumor}/${tumor}.f1r2.tar.gz \
		--germline-resource ${germlineresource} \
		--panel-of-normals ${ponvcf}\
		--native-pair-hmm-threads ${SLURM_JOB_CPUS_PER_NODE} \
		--tmp-dir ${outfolder}/${tumor}/${tumor}.tmp
	echo "=== DONE! ==="
	echo ""

	echo "=== learn read orientation model for sample ${tumor} ==="
	# read orientation
	gatk --java-options "-Xmx20G -XX:+UseParallelGC -XX:ParallelGCThreads=${SLURM_JOB_CPUS_PER_NODE}" \
		LearnReadOrientationModel \
		-I ${outfolder}/${tumor}/${tumor}.f1r2.tar.gz \
		-O ${outfolder}/${tumor}/${tumor}.readOrientationModel.tar.gz \
		--tmp-dir ${outfolder}/${tumor}/${tumor}.tmp
	echo "=== DONE! ==="
	echo ""
	
	echo "=== get pileup summaries for sample ${tumor} ==="
	# pileup
	gatk --java-options "-Xmx20G -XX:+UseParallelGC -XX:ParallelGCThreads=${SLURM_JOB_CPUS_PER_NODE}" \
		GetPileupSummaries \
		-I ${outfolder}/${tumor}/${tumor}.sorted.markDup.recal.bam \
		-O ${outfolder}/${tumor}/${tumor}.sorted.markDup.recal.pileupSummary.table \
		-V ${germlinecommon} \
		-L ${germlinecommon} \
		--tmp-dir ${outfolder}/${tumor}/${tumor}.tmp
	echo "=== DONE! ==="
	...
	echo "=== calculate contamination for sample ${tumor} ==="
	# calc contamination
	gatk --java-options "-Xmx20G -XX:+UseParallelGC -XX:ParallelGCThreads=${SLURM_JOB_CPUS_PER_NODE}" \
		CalculateContamination \
		-I ${outfolder}/${tumor}/${tumor}.sorted.markDup.recal.pileupSummary.table \
		-O ${outfolder}/${tumor}/${tumor}.sorted.markDup.recal.contamination.table \
		--tumor-segmentation ${outfolder}/${tumor}/${tumor}.sorted.markDup.recal.segments.table \
		--tmp-dir ${outfolder}/${tumor}/${tumor}.tmp
	echo "=== DONE! ==="
	echo ""

	echo "=== filter mutect2 variants for sample ${tumor} ==="
	# filter calls
	gatk --java-options "-Xmx20G -XX:+UseParallelGC -XX:ParallelGCThreads=${SLURM_JOB_CPUS_PER_NODE}" \
		FilterMutectCalls \
		-R ${refgenome} \
		-V ${outfolder}/${tumor}/${tumor}.soma.paried.vcf.gz \
		-O ${outfolder}/${tumor}/${tumor}.soma.paried.filtered.vcf.gz \
		--contamination-table ${outfolder}/${tumor}/${tumor}.sorted.markDup.recal.contamination.table \
		--tumor-segmentation ${outfolder}/${tumor}/${tumor}.sorted.markDup.recal.segments.table \
		--orientation-bias-artifact-priors ${outfolder}/${tumor}/${tumor}.readOrientationModel.tar.gz \
		--tmp-dir ${outfolder}/${tumor}/${tumor}.tmp
		echo "=== DONE! ==="
		echo ""
done
...
for file in ${bam_files[@]}; do
	germ=$(echo ${file[$i]} | rev | cut -d "/" -f1 | rev | cut -d "." -f1 | cut -d "_" -f1,2 ) # works!
	echo "=== call germline variants for sample ${germ} ==="
	# germline caller
	gatk --java-options "-Xmx20G -XX:+UseParallelGC -XX:ParallelGCThreads=${SLURM_JOB_CPUS_PER_NODE}" \
		HaplotypeCaller \
		-R ${refgenome} \
		-I ${outfolder}/${germ}/${germ}.sorted.markDup.recal.bam \
		-O ${outfolder}/${germ}/${germ}.germ.g.vcf.gz \
		-ERC GVCF\
		--native-pair-hmm-threads ${SLURM_JOB_CPUS_PER_NODE} \
		--tmp-dir ${outfolder}/${germ}/${germ}.tmp
	echo "=== DONE! ==="
	echo ""
	...
	gatk --java-options "-Xmx20G -XX:+UseParallelGC -XX:ParallelGCThreads=${SLURM_JOB_CPUS_PER_NODE}" \
		GenotypeGVCFs \
		-R ${refgenome} \
		-V ${outfolder}/${germ}/${germ}.germ.g.vcf.gz \
		-O ${outfolder}/${germ}/${germ}.germ.vcf.gz \
		--tmp-dir ${outfolder}/${germ}/${germ}.tmp # this runs per samples, loses group germline calling power. process downstream with GenomicsDBImport and GenotypeGVCFs next (different script)
	echo "=== Done! ==="
done
rm ${outfolder}/${germ}/${germ}.bam
rm ${outfolder}/${germ}/${germ}.sorted.ba*
rm ${outfolder}/${germ}/*fq.gz